# copilot-compare

## 概要

同じ課題を GitHub Copilot と Claude Code に解かせて、結果を突き合わせるための採点ハーネス。

課題は仕様書 1 枚（`spec.md`）と、エージェントには見せない隠しテストの組で定義してある。
両者に同じ仕様と同じリポジトリ指示を与え、実装だけを差し替えてテストを当てる。
「どちらが賢いか」ではなく、**同じ仕様書からどこを取りこぼすか**を見るのが目的。

計測は 2 段階に分けている。

| | 測るもの |
| --- | --- |
| **Phase A** | 仕様書だけを渡した一発勝負で、何点取れるか |
| **Phase B** | 落ちたテストを、どれだけ少ない情報で自力修復できるか |

Phase B ではヒントを 3 段階（件数だけ → テスト名だけ → pytest 出力）でしか出さず、
**満点に届いた段階**を記録する。実務では、エラー全文を貼るより
「動いてないよ」だけで直せる相手のほうが手離れが良い。その差を数字にしたい。

課題は `llm-api` テーマで実際に踏んだ箇所から採った。
`t1` は `/v1/qa` が返す SSE のデコード、`t2` は上流エラーのリトライ判断で、
どちらも境界条件が多く、仕様書の読み落としが合否に出やすい。

## 技術スタック

- Python 3.13 / pytest（採点は junit-xml を読んで集計）
- GitHub Copilot（VS Code 拡張・エージェントモード）
- Claude Code
- リポジトリ指示: `.github/copilot-instructions.md` / `.github/instructions/*.instructions.md` / `CLAUDE.md`

## 動かし方

```bash
cd projects/copilot-compare

python3 -m venv .venv
.venv/bin/pip install -r requirements.txt

# ハーネスの自己検証（参照実装が全通し、starter が 0 点になることを確認）
.venv/bin/python src/score.py --reference
```

API キーは不要。ここまでは Copilot が無くても動く。

### 1 回分を回す

```bash
# 作業ディレクトリを用意する（runs/copilot/t1-sse-parser/work/ ができる）
.venv/bin/python src/new_run.py --agent copilot --task t1-sse-parser

# → エージェントに spec.md と PROTOCOL.md、作業場所は work/ だけを渡す（Phase A）
# → 終わったら run.json の phase_a に実測値を書く

# 採点して Phase A を凍結する
.venv/bin/python src/score.py --agent copilot --task t1-sse-parser --freeze

# → 満点でなければ Phase B。L1〜L3 のヒントを段階的に出して直させ、都度採点する
.venv/bin/python src/score.py --agent copilot --task t1-sse-parser
```

`--freeze` は現在の実装を `phase_a/` に退避する。Phase B で上書きしても Phase A の点は残る。
採点結果は `docs/results.md` と `docs/results.json` に出る。
手順の詳細と、条件を揃えるためのルールは [PROTOCOL.md](PROTOCOL.md) を参照。

### 課題

| 課題 | 内容 | テスト数 |
| --- | --- | --- |
| `t1-sse-parser` | SSE の逐次パーサ。チャンク境界・改行コード・BOM の扱い | 33 |
| `t2-retry-policy` | 指数バックオフ + `Retry-After`（秒数 / HTTP-date）の解釈 | 50 |

## Copilot を使えるようにする

VS Code の拡張（`github.copilot` / `github.copilot-chat`）を入れて GitHub アカウントで
サインインする。`.vscode/extensions.json` に推奨として入れてあるので、
このリポジトリを開くと VS Code が導入を促す。

Copilot Free でもエージェントモードは使える（補完 2,000 回 / プレミアムリクエスト 50 回・月）。
このテーマの課題 2 つなら、その枠内で収まる。
なお GitHub は 2026-06-01 にプレミアムリクエストを AI Credits によるトークン課金へ切り替えているため、
枠の数え方は契約時点の表示を確認すること。

## 工夫した点・設計判断

- **合否を機械で決められる課題にした**。「良いコードを書けたか」は評価者の主観に落ちるので、
  仕様を隠しテストに翻訳できる題材だけを選んだ。SSE パーサもリトライ判断も、
  外部 I/O が無く、境界条件が仕様書に書き切れて、正解が一意に決まる。
  時間依存を避けるため、リトライ側は sleep せず「次に何秒待つか」を返す純粋な関数にし、
  ジッターと現在時刻は引数で注入する形にした。

- **両方のエージェントに同じ指示を読ませた**。VS Code の `chat.useClaudeMdFile` を有効にして、
  Copilot にもルートの `CLAUDE.md` を読ませている。
  比較したいのはエージェントの差であって、指示文の差ではないため。
  テーマ固有のルール（隠しテストを読まない等）は
  `.github/instructions/copilot-compare.instructions.md` に `applyTo` 付きで置き、
  `projects/copilot-compare/**` を触るときだけ効くようにした。
  内容は `PROTOCOL.md` と同じで、Claude Code 側は `PROTOCOL.md` を直接読む。

- **starter に空のシグネチャを置いた**。公開インターフェースを仕様書の文章だけで伝えると、
  引数名や戻り値の型がずれてテストがインポートすら通らず、
  「仕様を読み違えた」のか「呼び出し規約を外した」のかが混ざる。
  シグネチャを固定して、測りたい中身の差だけが出るようにした。

- **Phase A では失敗したテストをエージェントに返さない**。採点は作業完了後の 1 回だけ。
  途中でテスト結果を渡すと総当たりで通せてしまい、仕様書の読解力ではなく
  試行回数の勝負になる。実務でも、仕様書しか無い状態での初回実装の質が問題になる。

- **修復力は、点数ではなくヒントの段階で測ることにした**。Phase B で
  「何往復で直ったか」だけを見ると、ヒントの出し方次第でいくらでも変わってしまう。
  そこで与える情報を L1（件数だけ）/ L2（テスト名だけ）/ L3（pytest 出力）に固定し、
  **満点に届いた段階**を指標にした。手順が決まっているぶん、自己申告の往復数より比較に使える。
  段階を飛ばして L3 を出すと、そのエージェントが L1 で直せたのかが永久に分からなくなるため、
  飛ばさないことを PROTOCOL の側でルールにしている。

- **Phase A の実装を `phase_a/` に凍結してから Phase B に入る**。同じディレクトリで
  上書きさせると初回の点数が消え、「一発でどこまで行けたか」が残らない。
  採点側は `phase_a/` と現在の実装のバイト比較で Phase B の実施有無を判定するので、
  凍結直後に採点しても Phase B が「未実施」と正しく出る。

- **記録ファイルをエージェントの作業ディレクトリの外に出した**。当初は
  `runs/<エージェント>/<課題>/` に実装と `run.json` を同居させていたが、
  Phase A の失敗原因を `run.json` の `notes` に書いた結果、
  **その分析が Phase B の開始前にエージェントの context へ入り、
  L1（件数だけ）の計測が 1 件無効になった**。実装は `work/` に隔離し、
  `run.json` と `phase_a/` はその外に置く構成に変えた。
  計測ハーネスは、測る側が書いたものが被験側に見えない配置になっていないと成立しない。

- **合格数以外に行数・所要時間・往復回数も残す**。同点になったときに差が見えないため。
  ただし後ろ 2 つは自己申告で、厳密な指標ではない。`run.json` に手で記録する。

- **隠しテストを技術的に隠せていないことは、隠さず書いた**。リポジトリが公開されている以上、
  エージェントは `tasks/*/tests/` を読めてしまう。ルールで禁じ、会話ログで確認し、
  `run.json` の `used_spec_only` に記録する運用にした。
  ハーネスの限界を README に書かないと、後から見た数字を過大評価することになる。

## 検証で分かったこと

| エージェント | 課題 | Phase A | Phase B |
| --- | --- | --- | --- |
| copilot | t1-sse-parser | 32/33（1 往復 1 分） | 32/33（L3 まで使い切って未達） |
| copilot | t2-retry-policy | 50/50（1 往復 1 分） | — |
| claude-code | t1-sse-parser | 33/33（1 往復 1 分） | — |

**この数字を比較として読むときの前提**: Copilot は auto ルーティングで non-reasoning
モデルが選ばれ、Claude Code は Opus 5 で走っている。モデルを統制していないので、
これは「Copilot vs Claude Code」ではなく **「Copilot の既定 vs Claude Code の既定」**
の比較でしかない。各 1 本ずつ、`n=1` である点も含めて割り引いて読む必要がある。

### 課題の難しさは、分量ではなく「隠れた状態」で決まった

t2 は判定の順序・`Retry-After` の 2 形式・境界値と仕様書は長いが、Copilot は 1 往復 1 分で満点。
一方 t1 で唯一落ちたのは、`feed()` をまたいで持ち越す状態が要るチャンク境界の 1 箇所だった。
純粋関数は一発で書けて、ステートマシンが残る。

### 同じ壁に、違う当たり方をした

t1 の `\r` 境界に、両者とも当たっている。

| | 内容 | 点数 |
| --- | --- | --- |
| copilot | 境界を**認識せず**、処理を書かなかった | 32/33 |
| claude-code（1 回目） | 境界を**認識した上で**「持ち越し」方式を選択 | 31/33 |
| claude-code（2 回目） | 最初から「後追い」方式（`\r` で確定し次の `\n` を読み飛ばす） | 33/33 |

**より深く考えたほうが点数が低くなる区間がある。** 問題を無視すれば 1 件しか壊れないが、
中途半端に手を打つと 2 件壊れる。持ち越し（先送り）は `close()` が常に空を返す仕様と
噛み合わず、原理的に満点に届かない。そして Copilot が L1 のヒントで到達した設計は、
claude-code 1 回目のスタート地点と同一だった。

claude-code の 1 回目 31/33 と 2 回目 33/33 は同一モデル・同一プロンプトである。
**run 間の分散がこの程度ある**ということ自体が、1 本の数字で優劣を語れない理由になっている。

### 修復力は、初回の点数から予測できない

Copilot の t1 Phase B は L1 から L3 まで使い切って未達だった。

| | 点数 | 中身 |
| --- | --- | --- |
| Phase A | 32/33 | チャンク境界の処理なし |
| L1（件数のみ） | 31/33 | 末尾 `\r` を保留する方式に変更。原理的な袋小路 |
| L2（テスト名） | 31/33 | 正しい `_pending_cr` を追加。ただし L1 の `break` を残したため到達不能な死にコードに |
| L3（pytest 出力） | 32/33 | `break` を除去したが、同時に `_pending_cr` の使用も除去。Phase A と等価に戻る |

L1 の時点で原因箇所そのものには到達していた（件数だけの情報で `_next_line` に手を入れた）。
効かなかったのは、**前の修正を残したまま次の修正を積む**ことによる。
L2 では新旧の修正が互いに矛盾し、L3 ではその矛盾を解く際に新しい方を捨てた。
`close()` に読まれない `self._pending_cr = False` だけが残骸として残っている。

一発勝負 1 分で 32/33 という数字と、6 分 4 往復かけて動かなかった数字は、
どちらも同じくらい実務的な情報だと考えている。前者だけを見て採用を決めると、
「ほぼ合っているが最後の 1 件が残る」状況のコストを見落とす。

### 仕様書に「書いたつもりで書いていない」箇所が露出する

t2 は当初 49/50 で、落ちたのは `Retry-After: "  3  "` の前後の空白を取り除くかどうか。
仕様書は「ASCII 数字のみ」としか書いておらず、空白の扱いはどこにも書いていなかった。
Copilot は仕様通りに解釈してバックオフへ落としており、**参照実装が勝手に `.strip()` していた**
こちらの不備だった。テストを仕様に合わせ、参照実装から `.strip()` を除去して 50/50。
`spec.md` 側は変更していない（例を足すと、後から走らせるエージェントだけが有利になるため）。

人間相手なら「まあ trim するだろう」で流れる曖昧さが、そのまま差分として出てくる。

### 自己検証は、自分の設計が壊れる形を系統的に避ける

claude-code の 1 回目は 123 行・29 検査の `selfcheck.py` を書いて実行していた
（PROTOCOL ルール 4 の範囲内。Copilot はどちらの課題でも書かなかった）。
CR 分割もちゃんと検査していたが、**全ケースが `\n` で終わる入力**で、
末尾が裸の `\r` になる入力を 1 件も含んでいなかった。
自分が「持ち越しで大丈夫」と考えた前提の外側を、検証でも踏まなかったことになる。

### ハーネス側の教訓

- **境界条件のテストは、当てる位置を変えないと素通りする。**
  チャンク境界のテストは 3 件あるが、Copilot の Phase A で実際に欠陥を検出したのは 1 件だけ。
  余計な空行が「データバッファが空の位置」に落ちると dispatch が起きず実害が出ないため、
  残り 2 件はまぐれで通っていた。イベント種別を挟んだ 1 件だけが差を見た。

- **エージェントは放っておくとテストを実行しようとする。** Copilot は t1 の Phase B 中に
  `pip install --user pytest && pytest` を要求してきた。許すと `pytest` の出力を
  自力で入手でき、ヒント段階の計測が成立しない。PROTOCOL と instructions ファイルの
  両方で禁止したところ、次の新規セッション（t2）では要求してこなくなった。

- **測る側が書いたものは、被験側から見えない場所に置く。** `run.json` を作業ディレクトリ内に
  置いていたため、`notes` に書いた Phase A の失敗原因が Phase B の開始前に
  エージェントの context へ入り、L1 の計測が 1 件無効になった（エージェント自身が申告した）。
  実装を `work/` に隔離し、記録はその外へ出した。

- **所見を書いた時点で、その課題はそのリポジトリでは焼ける。** 上の「検証で分かったこと」には
  t1 の失敗原因と正解の方向が書いてある。`git log` を辿れば過去の実装も参照実装も読める。
  claude-code の t1 を測り直した際、この経路を塞げていなかった。
  **計測ハーネスを成果物として公開することと、同じ課題を何度もクリーンに測ることは両立しない。**
  再測定が要るなら、所見を書く前に済ませるか、課題を作り直す。

## 成果物

- `src/` — 採点ハーネス（`new_run.py` で作業場所を用意、`score.py` で採点）
- `tasks/` — 課題 2 件（仕様書 / starter / 隠しテスト 83 件 / 参照実装）
- `runs/<エージェント>/<課題>/` — `work/`(実装) / `phase_a/`(凍結) / `run.json`(計測メタ)
- `docs/results.md` — 採点結果（`score.py` が生成。手で編集しない）
- `PROTOCOL.md` — 条件を揃えるための手順
