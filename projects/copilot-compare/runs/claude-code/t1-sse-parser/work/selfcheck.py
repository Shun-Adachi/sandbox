...
# 1. 裸の \r で終わるチャンク/ストリーム。\r だけで行は完成している
p = SSEParser()
check("bare cr ends block", p.feed("data: x\r\r"), [SSEEvent(event="message", data="x", id=None)])
p = SSEParser()
check("cr then crlf", p.feed("data: x\r\r\n"), [SSEEvent(event="message", data="x", id=None)])
p = SSEParser()
check("crlf then cr", p.feed("data: x\r\n\r"), [SSEEvent(event="message", data="x", id=None)])
# 持ち越した \r のあとに空 feed が挟まっても壊れない
p = SSEParser()
check("cr then empty feed", p.feed("data: x\r"), [])
check("cr then empty feed 2", p.feed(""), [])
check("cr then empty feed 3", p.feed("\n\r"), [SSEEvent(event="message", data="x", id=None)])
# \r が行の区切りとして連続する
p = SSEParser()
check(
    "cr separated events",
    p.feed("data: a\r\rdata: b\r\r"),
